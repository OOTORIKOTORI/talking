      ...
      <div class="rounded-lg overflow-hidden shadow-2xl bg-black relative">
        ...
        <div v-if="!fullscreen" class="absolute inset-0 pointer-events-none">
          ...
          <TestPlayPanel
            ...
            variant="normal"
            :current-scene-label="testPlayCurrentSceneLabel"
            :current-node-label="testPlayCurrentNodeLabel"
            :next-node-label="testPlayNextNodeLabel"
            :choice-count="choices.length"
            :fast-confirm-mode="testPlayFastConfirmMode"
            :material-summary="testPlayMaterialSummary"
            :choice-transitions="testPlayChoiceTransitions"
            :transition-logs="testPlayTransitionLogs"
            :return-to-editor-to="returnToEditorTo"
            :format-transition-log-line="formatTestPlayTransitionLogLine"
...
function exportTestPlayTransitionLogsJson() {
  ...
  try {
    ...
    const logData = testPlayTransitionLogs.value.map((log) => ({
      ...
      toLabel: log.toLabel,
      choiceIndex: log.choiceIndex ?? null,
