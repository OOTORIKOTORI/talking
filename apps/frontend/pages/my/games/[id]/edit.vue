...
const publishCheckTopIssues = computed(() => {
  ...
  return scenarioCheckIssues.value.filter((i) => i.severity === 'error').slice(0, 3)
})

const publishCheckTopIssuesRemainder = computed(() => {
  if (publishCheckStatus.value !== 'error') return 0
  return Math.max(0, scenarioCheckCounts.value.error - 3)
})
            ...
            <div v-if="sectionOpen.scenarioCheck" class="border-t border-gray-200 px-3 py-2">
              ...
              <div class="mb-3 rounded-lg border px-3 py-3 text-sm" :class="publishCheckStatusCardClass">
                <div class="flex items-start justify-between gap-2">
                  ...
                  <a
                    v-if="scenarioCheckTotalCount > 0"
                ...
                <div class="mt-2 flex flex-wrap gap-2 text-[11px]">
                  <span v-if="scenarioCheckCounts.error > 0" class="rounded border border-red-200 bg-red-50 px-2 py-0.5 text-red-700">要修正 {{ scenarioCheckCounts.error }}件</span>
                  <span v-if="scenarioCheckCounts.warning > 0" class="rounded border border-amber-200 bg-amber-50 px-2 py-0.5 text-amber-700">注意 {{ scenarioCheckCounts.warning }}件</span>
                  <span v-if="scenarioCheckCounts.info > 0" class="rounded border border-slate-200 bg-slate-50 px-2 py-0.5 text-slate-600">情報 {{ scenarioCheckCounts.info }}件</span>
                </div>
                <!-- カテゴリ別件数 -->
                <div v-if="scenarioCheckTotalCount > 0" class="mt-1 flex flex-wrap gap-2 text-[11px] text-slate-500">
                  <span v-if="scenarioCategoryCounts.structure > 0">構成 {{ scenarioCategoryCounts.structure }}件</span>
                  <span v-if="scenarioCategoryCounts.assetReference > 0">素材参照 {{ scenarioCategoryCounts.assetReference }}件</span>
                  <span v-if="scenarioCategoryCounts.characterReference > 0">キャラクター参照 {{ scenarioCategoryCounts.characterReference }}件</span>
                </div>
                <!-- 優先問題 (要修正がある場合、最大3件) -->
