...
> 最終更新: 2026-05-04（アセット削除時の利用影響表示MVP）
...
ゲーム制作機能の基盤が整い、MVP級の編集・公開・プレイが一通り動く状態。参照診断API（エディタ検証）を追加。アセット削除前の影響警告表示を追加。
...
**将来課題: お気に入り / 素材棚 / 引用 — 概念分離**

現在 `favorites` がお気に入りと制作素材棚を兼任している。将来的に以下の概念・DB分離を検討すること。

- **likes**（お気に入り/いいね）
  - 純粋な好き/応援/評価指標
  - ランキング・おすすめへの活用
- **asset_library / asset_shelf**（アセット素材棚）
  - 制作用に保存したアセット
  - `AssetPicker` の素材候補として表示
- **character_library / character_shelf**（キャラクター素材棚）
  - 制作用に保存したキャラクター
  - `CharacterPicker` の素材候補として表示
- **asset_usages**（アセット引用/使用）
  - 実際にゲーム内ノード/シーンで参照されているアセット
  - スタッフロール/クレジット・使用数集計の基盤
- **character_usages**（キャラクター引用/使用）
  - 実際にゲーム内で参照されているキャラクター

指標分離案:
- いいね数（人気/評価）
- 素材棚追加数（制作者が使いたいと思った数）
- 使用数/採用数/引用数（実際にゲーム内で使われた数）
- 閲覧数（見られた数）

UI文言整理:
- 「お気に入りに追加」→ 好き/応援/あとで見る
- 「素材棚に追加」→ 制作素材候補として保存
- 「このゲームで引用している素材」→ ゲーム内で実際に参照している素材
- 「クレジットに表示」→ スタッフロール/素材クレジット対象
...
- ~~アセット削除/非公開化時の「利用中ゲームへの影響表示」導線~~ → 削除時影響表示MVPは実装済み（2026-05-04）
...
- `AssetUsage` / `GameAssetReference` テーブル導入による参照関係の明示的記録
...
- 素材参照のシナリオチェック warning（bg/BGM/SE/キャラ全体）
- アセット/キャラクター削除・非公開化時の利用中ゲームへの影響表示
- お気に入り/素材棚/引用のDB・UI分離（制作素材棚 vs 単純いいね の概念分離）
