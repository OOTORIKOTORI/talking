...
> 最終更新: 2026-05-21（FI-057 edit画面情報設計v2 MVP-1 — 通常表示ミニプレビュー上部配置。sticky は後続候補）
...
- **FI-057 edit画面情報設計v2 MVP-1（2026-05-21）**: 通常表示の右ペイン上部（操作ボタン群の直後）にミニプレビューを移動。編集を始める前にプレビューが見える位置への配置を試す。高さ 200px・「隠す/表示」トグル付き。sticky 固定は右ペインの `overflow-y-auto` との相性で期待通りに動作しないため外した（スクロール追従は後続候補）。全画面表示側は既存挙動維持。
