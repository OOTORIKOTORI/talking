...
> 最終更新: 2026-05-01（NodePicker 残課題整理・ゲームプレイ画面キーボード操作将来課題追記）
...
#### ゲームプレイ画面のキーボード操作
ノベルゲーム/ADVとしてゲームプレイ画面をキーボードで進行できるようにする。PC向け操作としてまず実装し、スマホ/タブレット対応とは分けて扱う。
...
- 数字キー `1`〜`9`:
  - 対応する番号の選択肢を選ぶ案を検討
- `Esc`:
  - セーブ/ロードなどのモーダルが開いていれば閉じる

注意点:
- `input` / `textarea` / `select` / `button` にフォーカスがある場合はショートカットを邪魔しない
- IME 変換中は `Enter` / 矢印キー処理で誤爆しない
- セーブ/ロードモーダル表示中はモーダル操作を優先する
- ブラウザ標準の `Space` スクロールとの干渉に注意する
- PC 向け操作としてまず実装し、スマホ/タブレット対応とは分けて扱う
- 難易度: 中
