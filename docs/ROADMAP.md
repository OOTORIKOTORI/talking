...
> 最終更新: 2026-05-17（FI-055 Phase 2-b-2 + Phase 2-c + 話者キャラクリア表示修正 反映）
...
最新仕様確認コミット: de268a27a296dd010869df3b90bea052a44701a1
...
- edit 画面プロパティフォームの共通コンポーネント化（通常表示/全画面表示の二重実装解消）
	...
	- 方針: 一括改修ではなく段階移行（Phase 1 演出系 → Phase 2 遷移/分岐 → Phase 3 素材/キャラ参照 → Phase 4 保存/危険操作/公開前チェック周辺）
	- 実装状況: Phase 1 演出系フォーム共通化MVPは実装済み（`NodeEffectsFields.vue` を通常表示/全画面表示の両方で再利用）
	- 残課題: Phase 2 以降（遷移/分岐、素材/キャラ参照、保存/危険操作/公開前チェック周辺）の共通化
