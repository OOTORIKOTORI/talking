...
ゲーム制作機能の基盤が整い、MVP級の編集・公開・プレイが一通り動く状態。参照診断API（エディタ検証）を追加。アセット削除前の影響警告表示を追加。いいね / 素材棚 / 採用 / 引用・クレジットの4概念分離設計を docs に明文化。

**実装済み（主要）**
- アセット削除時の利用影響表示MVP（`GET /assets/:id/usage-impact` API、削除確認モーダルへの影響表示統合、他人ゲームは件数のみ、100件超でも全件返さない設計）
- いいね / 素材棚 / 採用 / 引用・クレジット の4概念分離設計を docs に明文化（`docs/PROJECT_SPEC.md` 「お気に入り / 素材棚 / 引用 — 概念整理（設計方針）」、コード実装は将来段階的に実施）
...
**将来課題: いいね / 素材棚 / 採用 / 引用・クレジット — 概念分離**

現在 `favorites` がいいねと制作素材棚を兼任している。将来的に以下の4概念・DB分離を検討すること。設計方針は `docs/PROJECT_SPEC.md` の「お気に入り / 素材棚 / 引用 — 概念整理（設計方針）」に詳細を明文化済み（2026-05-04）。
...
**直近の残課題（優先順）**
- 公開前 warning 強化 / UI改善（削除済み/非公開化素材の参照切れ検知はAPI実装済み、フロントUI改善が残り）
- キャラクター削除時の利用影響表示（削除時影響表示API の `GameAssetReference` 相当。アセット削除時MVPは実装済み（2026-05-04）、キャラクター削除時は未実装）
...
- キャラクター削除時の利用中ゲームへの影響表示（アセット削除時MVPは実装済み、キャラクター削除時は未実装）
