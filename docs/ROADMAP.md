...
> 最終更新: 2026-05-13（キャラクター側の一覧UI統一MVP・素材詳細/キャラクター詳細レイアウト統一MVP・公開キャラクター一覧の検索/タグ/並び替え追加・キャラクター管理の検索/公開状態フィルタ/タグ/並び替え追加・キャラクター管理カードの公開/非公開バッジ追加）
...
- **素材・キャラクターカードUI表示情報統一MVP**（2026-05-13 実装）
	- 公開素材カードにタグ表示を追加し、説明未入力時の余白崩れを抑制
	- 公開キャラクターカードを素材カード寄りに整理し、説明・作者リンク・クレジット表記バッジ・タグ・お気に入りボタンを表示
...
1. **コンテンツ名称統一 - UI 完全統一フェーズ（後続）** — 2026-05-13 の名称ルールMVP（見出し・タブ・ナビの統一、docs更新）の後続として、以下を段階実装
	- **素材管理・キャラクター管理カードの操作導線 polish** — 編集ボタン配置・カード内クリック領域・操作フィードバックの最終調整
