...
最新仕様確認コミット: `abc5af18e973a33c8dda0fdc0c780bea9c67bda5`
...
3. **スタッフロール拡張 / BGM（P2〜P3）** — 現スタッフロールMVP（速度設定実装済み）の拡張（表示順カスタマイズ、自動表示、BGM、より細かな速度設定）。音声素材/クレジット設計と絡むため慎重に扱う。
...
- スタッフロール拡張（自動表示ON/OFF、表示順カスタマイズ、終了時挙動、より細かな速度カスタマイズ、スタッフロール用BGM）
...
- 将来拡張として、作者がスタッフロール演出を設定できる余地を整理する（自動再生ON/OFF、速度、終了時挙動、BGMソース、音量、エンディング後自動表示）。
