...
3. **作者向けテストプレイ支援（P1〜P2）** — 作者の確認コストを直接下げるため、ゲーム規模が大きくなる前に検討価値が高い。
4. **スタッフロール設定 / BGM（P2〜P3）** — 現スタッフロールMVPの拡張。音声素材/クレジット設計と絡むため慎重に扱う。
5. **公開版/下書き版分離** — 公開中編集の安全性向上の本命施策（反映タイミング分離）。
...
- 作者向けテストプレイ支援（公開プレイヤー向けUIと分離した作者専用導線）
- テストプレイ中のデバッグ表示（現在シーン/ノード、次ノード、選択肢遷移先、使用中素材/キャラクター）
...
#### 作者向けテストプレイ支援（将来課題）
公開プレイヤー向け本番UIとは分離し、作者の確認コストを下げるための専用支援として段階導入を検討する。

優先度目安:
- P1: 編集画面で選択中のシーン/ノードからテストプレイ開始
- P1: テストプレイ中の現在シーン/現在ノード表示
- P2: 遷移ログ表示
- P2: テキスト即表示 / 高速確認モード
- P3: フラグ/変数インスペクタ
