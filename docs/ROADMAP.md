...
> 最終更新: 2026-05-12（開始ノード到達不能判定見直しMVP・スタッフロール表示順カスタマイズMVP・スタッフロール自動表示ON/OFF MVP・スタッフロール速度設定MVP・スタッフロール設定MVP・スタッフロール終了時挙動MVP・右ペイン軽量化MVP・クレジット/スタッフロール設定タブ保存polish・公開中ゲームの全体設定保存confirm統一MVP・作者向けテストプレイ支援・パネルUI整理・テストプレイパネル小コンポーネント化MVP・開始位置フォールバック修正）
...
- 削除系は公開中のみ、公開中警告 + 参照summary を1回のconfirmに統合して表示（通常削除confirmとの二重表示を回避）。
- 非公開ゲームでは公開中向けconfirmは出さず、削除系は従来の削除confirmのみ表示。
...
1. **コンテンツ名称統一 - UI 完全統一フェーズ（後続）** — 2026-05-13 の名称ルールMVP（見出し・タブ・ナビの統一、docs更新）の後続として、以下を段階実装
   - **素材・キャラクター詳細ページのレイアウト統一** — `素材詳細` と `キャラクター詳細` の両ページ構成・セクション配置を統一
   - **素材・キャラクターカードUIの表示情報統一** — 一覧カード・管理カードで表示する情報項目（作者表示、作成日、公開状態、説明）を統一
   - **素材管理・キャラクター管理の検索/フィルタ統一** — 素材管理にある検索/公開状態フィルタをキャラクター管理にも展開、両者の検索UXを統一
   - **所有者向け管理エリアの表現統一** — `/my/assets` と `/my/characters` の構成・表現・操作導線（作成・編集・削除・公開状態変更）を統一
   - **公開側ギャラリーの検索/フィルタ統一** — 素材・キャラクター一覧それぞれに検索/フィルタを均衡配置する検討
...
- 削除/参照整合: `GameChoice.targetNodeId` の nullable 化。ノード参照切れ警告・到達不能ノード検出は後続MVPで実装済み
...
- P2: ノード参照切れ警告・到達不能ノード検出（当時メモ。後続MVPで実装済み）
- P2以降: フローチャート表示
...
- 到達不能ノード検出は実装済み。開始ノード到達不能判定見直しMVPで、ゲーム開始ノード + 各シーン開始ノードを到達起点として扱うようにした。
...
#### ノード参照切れ警告・到達不能ノード検出
> 注: 下記は当時の検討メモ。後続MVPでノード参照切れ警告・到達不能ノード検出・開始ノード到達不能判定見直しMVPまで実装済み。到達可能性の起点はゲーム全体開始ノード + 各シーン開始ノード。孤立ノード検出は維持し、残課題はフローチャート可視化、意図的未使用ノードの扱い、チャプター/隠しシーン/任意開始点などの高度な到達性ルール。

ノード削除や分岐編集に伴う整合崩れを減らすため、参照切れ警告と孤立ノード検出を追加する計画として整理していた。
...
#### スタッフロール / クレジット表示
2026-05-09 時点で **スタッフロールUI MVP は実装済み**。
- 表示データは既存 `GET /games/:id/credits` を利用（手動クレジット・使用素材・使用キャラクター）
- 導線は「プレイ終了画面」と「公開ゲーム詳細ページ」から開く
- DB変更・migration追加・API追加なし（UI MVP時点）
- 自動スクロール（初期ON・最下部停止）、停止/再開、先頭へ戻る、手動操作時の自動停止は実装済み
- スクロールバーを目立たせない表示、上下フェード、再生状態表示、中央上映寄りレイアウトは実装済み

後続MVPで実装済み:
- 速度設定（ゆっくり/標準/速い）
- エンディング後自動表示ON/OFF（`staffRollAutoOpenEnabled`）
...
設計メモ（将来課題）:
- スタッフロールUI MVP（手動クレジット/使用素材/使用キャラクター、既存 `GET /games/:id/credits` 利用）は実装済みのまま維持する。
- 将来拡張として、作者がスタッフロール演出を設定できる余地を整理する（個別アイテム表示順、終了時挙動、より細かな速度、BGMソース、音量など）。
- スタッフロール用BGMは、音声素材・権利表示・`GameCredit`・公開前チェックとの整合確認を前提に検討する。

#### 作者向けテストプレイ支援（MVP実装済み + 将来課題）
公開プレイヤー向け本番UIとは分離し、作者の確認コストを下げるための専用支援を段階導入する。

実装済み（MVP）:
- 編集画面で選択中のシーン/ノードからテストプレイ開始（`testPlay=1` を付与）。
- テストプレイ中の現在シーン/現在ノード表示。
- 次ノード・選択肢数・選択肢遷移先概要の表示。
- 現在ノードの使用素材ID（背景/BGM/SFX）とキャラクター数の表示。
- テストプレイパネルの折りたたみ/展開。
- テストプレイパネルの情報整理MVP（現在地 / 操作 / ノード情報 / 遷移ログの4セクション化、通常/フルスクリーンで同一構造）。
- 編集画面へ戻る導線（sceneId/nodeId クエリ付き）。
- 「選択肢までスキップ」MVP（作者本人の `testPlay=1` のみ表示）。
- 「選択肢までスキップ」は `nextNodeId` を連続でたどって最初の選択肢ノードで停止し、到達先本文をタイプライター待ちなしで即全文表示してから選択肢を表示する。選択肢の自動選択はしない。
- スキップ中の遷移ログは既存の `next` / `end` / `missing` を利用し、ボタン押下自体は記録しない。
- スキップ操作はメモリ保持のみで、DB / API / localStorage には保存しない。
...
設計方針メモ:
- 本番プレイの没入感を壊す情報（デバッグ表示・警告表示）はテストプレイ時に限定する。
- 公開プレイ体験と作者テストプレイ体験を混同しない。
