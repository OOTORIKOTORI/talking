...
> 最終更新: 2026-05-18（FI-056 演出セクション要約表示 MVP 反映）
...
- 右ペインセクションの要約表示（閉じた状態での情報把握）の強化
	- 実装状況: 基本情報（NodeBasicInfoFields.vue）・表示・素材（NodeMaterialsFields.vue）・演出（edit.vue）MVP実装済み（2026-05-18）
	- 残課題: 遷移・分岐等の他セクションへの展開
