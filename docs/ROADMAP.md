...
> 最終更新: 2026-05-23（アップロード時の画像寸法/16:9推奨チェックMVP）
...
最新仕様確認コミット: 4f2ba75dd08815e76ac536de6e89e4beaf4b7c33（素材カード・管理一覧に利用条件/クレジット要否バッジを追加）
...
- **FI-057 通常表示ミニプレビュー上部配置MVP 試行・不採用（2026-05-21）**: 通常表示の右ペイン内でプレビューを上部配置・sticky 固定する案を試みたが不採用。右ペイン幅との相性でプレビューが横長・低身長になり制作中プレビューとして見づらく、sticky も `overflow-y-auto` との相性で機能しなかった。コードは元に戻した。次は通常表示を無理に制作モード化せず、全画面表示を「制作モード」として磨く方向を検討する。
- **次の開発候補**: 全画面表示の制作モード化（導線改善・前回の表示モード保存）・テストプレイ高速確認まわりなど
...
- **アップロード時の画像寸法/16:9推奨チェックMVP**（2026-05-23 実装）`upload.vue` で画像ファイル選択時にクライアント側で寸法を読み取り、幅×高さと縦横比を表示。種別が背景（`IMAGE_BG`）/一枚絵（`IMAGE_CG`）のとき16:9との差分で OK/注意メッセージを非ブロッキング表示。音声ファイルには非表示。DB/API/migration 変更なし。
