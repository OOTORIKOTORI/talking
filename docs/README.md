...
進捗同期の基準は [ロードマップ](./ROADMAP.md) です。ハンドオフ文書は補助資料として扱います。

### [未着手アイデア台帳 (IDEA_BACKLOG)](./IDEA_BACKLOG.md)
未着手・一部実装・後続拡張アイデアの索引。ROADMAPに埋もれやすい案を確認する入口。

### [ロードマップ](./ROADMAP.md)
実装済み機能・既知の課題・次にやることを管理する正ドキュメント

### [ファイルマップ](./file-map.md)
フロントエンド・API・Workerの全ファイル一覧と各ファイルの役割・props/emit/関数名

### [ハンドオフ手順](./handoff.md)
旧メモ・補助資料。複数チャットにまたがる開発での引き継ぎテンプレート
