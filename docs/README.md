...
- 画像/音声のアップロード、署名付き配信
- アセット管理 / お気に入り
- **キャラクター管理**：立ち絵アップロード・編集、感情ラベル・パターン・並び順の管理
- **ゲーム制作（β）**：
  - シーン/ノード編集、背景・BGM・立ち絵配置、カメラ（倍率/位置）編集
  - プレビュー（通常/全画面、同一スケール保証）
  - 開始ノード指定、次ノード選択（NodePicker + Ctrl/⌘+K 起動）
  - 「保存して次のノードへ」（Ctrl/⌘+Enter、コピー項目選択）
  - メッセージウィンドウテーマ（v2: 1〜10段階プリセット、RGBA色、コントラスト警告）
  - テストプレイ（音声同意オーバーレイ、フルスクリーン対応）
