...
```text
docs/
├── IDEA_BACKLOG.md                          # 未着手/一部実装/実装済み/後続拡張アイデアの索引。ROADMAPで埋もれやすい案の入口
```
...
```text
apps/frontend/pages/
├── index.vue                                # トップページ。見つける/公開ゲーム/制作/アップロードへの主要導線、新着ゲーム/素材/キャラクター枠、控えめなAPI状態表示を扱う
...
├── upload.vue                               # 素材アップロード画面。画像/音声の投稿入口（公開/非公開の初期設定・利用条件入力を含む）。画像選択時に寸法を読み取り表示し、背景/一枚絵（IMAGE_BG/IMAGE_CG）では16:9比率の推奨チェックを非ブロッキングで表示
├── assets/
...
├── my/
│   ├── assets/
│   │   └── index.vue                        # 自分の素材管理一覧。投稿物の確認・整理用（公開/非公開・クレジット要否・利用条件有無バッジ・フィルタ対応）
│   ├── characters/
...
│       └── [id]/
│           └── edit.vue                     # ゲームエディタ。シーン/ノード/演出/テーマ設定を編集。右ペインに制作ガイドカード（折りたたみ・非表示・再表示可。2026-05-16 polish でコンパクト化）と公開前チェックパネル（サマリーカード・severity別件数・優先issue・カテゴリフィルタ・issue一覧）を内包。右ペイン上部アクションは表示/設定/出力で軽くグルーピングし、狭い幅では折り返し表示。表示設定リセット導線（3ペイン幅・右ペイン開閉・最後の選択位置・制作ガイド非表示状態）あり。基本情報フォームは `NodeBasicInfoFields.vue`（台詞/継続チェック/話者キャラ/話者表記）へ移譲済み。表示・素材（背景/BGM/SE）フォームは `NodeMaterialsFields.vue` へ移譲済み。キャラクター配置UIは `NodePortraitsFields.vue` へ移譲済み。遷移・分岐フォームは `NodeTransitionFields.vue`（次ノード/コピー対象）と `NodeChoicesFields.vue`（選択肢UI）へ段階移譲済み。保存ボタンは `NodeSaveActions.vue`、危険操作セクションは `NodeDangerZone.vue` へ移譲済み。保存payload正規化は `buildNodePayloadForSave` ヘルパーに一元化。NodePicker本体・選択状態・保存処理・`selectedCharLabel` computedは edit.vue 側責務を維持。段階的共通化計画は `docs/editor-property-form-refactor-plan.md` を参照
```
...
├── asset/
│   └── AssetCard.vue                        # 素材一覧用カード。詳細遷移（サムネイル/タイトル）と作者リンク・お気に入り操作をDOM構造で分離しつつ、説明・タグ・クレジット表記・利用条件あり有無・お気に入り件数を表示。お気に入り成功時はfavorite-toggledをemit（API変更なし）
├── character/
...
│   ├── NodeMaterialsFields.vue              # ノード編集画面の「表示・素材」セクション（背景/BGM/SE）共通コンポーネント。bgUrl/musicTitle/musicUrl/sfxUrlをpropsで受け取り、選択・クリアをemitで通知。AssetPicker本体は edit.vue 側責務を維持
