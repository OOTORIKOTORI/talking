...
- 作者向けテストプレイ支援（将来拡張、作者専用UI）
  - 公開プレイヤー向け本番UIとは分離し、テストプレイ時のみデバッグ表示を出す。
  - 想定項目: 選択中シーン/ノードから開始、現在シーン/ノード表示、次ノード/選択肢遷移先表示、使用中素材/キャラクター表示、遷移ログ、テキスト即表示/高速確認、分岐までスキップ、テストプレイ専用警告（開始地点未設定・参照切れ等）。
  - 将来的にフラグ/変数/好感度等を導入する場合は、`flags` / `variables` / `visitedNodes` / `affinity` などの状態インスペクタを検討する。
