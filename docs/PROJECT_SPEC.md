...
- 公開ゲーム
  ...
  - クレジット取得: `GET /games/:id/credits`
    ...
    - `ownerId` 表示は暫定（`by ${ownerId}`）。将来はプロフィール表示名/クリエイター名へ差し替え予定
...
- `ownerId` の `by ...` 表示は暫定運用。将来はプロフィール表示名/クリエイター名へ差し替える。
...
- 将来的にはプロフィール/クリエイター表示名へ差し替える方針は維持。
...
- 現状:
  - `ownerId` を `by <ownerId>` として暫定表示していたが、2026-05-05 UI polish にて `by d7ef...f292` 形式に短縮済み。
  - 用途バッジ化・行表示改善も同時に実施済み。
- 将来案:
  - ユーザープロフィール/クリエイター表示名を導入し、`by <displayName>` に差し替える
  - プロフィール未設定時は短縮UUID（現行の表示）にフォールバックする
  - リンク可能なら作者プロフィールページへリンクする
  - 削除済み/退会済み/非公開ユーザーの場合のフォールバック表示を決める
- プロフィール/クリエイター名の実装は将来段階で行う
