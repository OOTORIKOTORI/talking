...
- ~~公開前確認画面内の修正候補表示 MVP~~ → **実装済み（2026-05-07）** — クレジット確認モーダルの異常status項目に修正候補ボックスを表示。素材 `missing/deleted`、キャラクター `missing/deleted/private` に対応。最初の該当issueへの直接ジャンプMVP（2026-05-12）は実装済み。複数箇所一覧・入力フィールドへのピンポイントフォーカス・一括修正・自動差し替えは将来課題。
