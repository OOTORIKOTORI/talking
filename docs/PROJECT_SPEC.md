...
- クレジット確認からの直接ジャンプ拡張（最初の該当issueへの直接ジャンプMVPは実装済み。複数箇所一覧、入力フィールドへのピンポイントフォーカス、一括修正、自動差し替えは将来課題）
...
- より精密なノード/フィールド単位ジャンプ（最初の該当issueへの直接ジャンプMVPは実装済み。複数箇所一覧・入力フィールドへのピンポイントフォーカスは将来課題）
...
- 将来課題:
  - **作者プロフィール系**
    ...
    - プロフィール画像・SNSリンク
  - **クレジット運用系**
    - より精密なノード/フィールド単位ジャンプの拡張（最初の該当issueへの直接ジャンプMVPは実装済み。複数箇所一覧・入力フィールドへのピンポイントフォーカスは将来課題）
