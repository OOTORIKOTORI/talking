...
- ~~公開前チェックサマリーMVP~~ → **実装済み（2026-05-14）** — 公開前チェックパネル（編集画面右ペイン）の issue 一覧より上にサマリーカードを追加。全体状態として `公開準備OK` / `注意あり` / `要修正` を表示（`error > 0` → 要修正、`warning > 0` → 注意あり、両方 0 → 公開準備OK、参照診断読込中 → チェック中…）。severity 別件数（要修正 N件 / 注意 N件 / 情報 N件）と既存カテゴリ別件数（構成 / 素材参照 / キャラクター参照）を表示。要修正がある場合は error 系 issue を最大 3 件表示し、3 件超は「ほか N件」で省略。issue 一覧への `#publish-check-issues` アンカー導線付き。既存フィルター・カテゴリフィルター・「対象へ移動」導線は維持。自動修正・一括修正・入力フィールドへのピンポイントフォーカスは未実装（将来課題）。DB/API変更なし。
