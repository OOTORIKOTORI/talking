...
#### 2. 素材棚 / 制作棚（Asset Shelf）

| 項目 | 内容 |
|------|------|
| 意味 | 自分のゲーム制作で使うために保存する・Pickerに出す素材を管理する |
| 用途 | 背景/BGM/SE/カバー画像の選択候補、キャラクター配置候補、制作用ブックマーク、自分の制作素材管理 |
| 候補名 | 素材棚 / 制作棚 / 制作に追加 / 素材として使う / ライブラリに追加 |
| 将来DB候補 | `asset_library` / `asset_shelf`、`character_library` / `character_shelf` |
| 備考 | 全公開素材をPickerに直接出すのではなく、棚に入れた素材だけを表示する方式 |

#### 3. 引用 / 使用 / 採用（Usage / Credit）

| 項目 | 内容 |
|------|------|
| 意味 | 実際にゲーム内で参照されている・クレジット/スタッフロールに載る・使用実績 |
| 用途 | このゲームで引用している素材、素材クレジット、スタッフロール、使用数/採用数/引用数、アセット作者への利用実績表示 |
| 候補名 | 引用 / 使用 / 採用 / クレジット対象 |
| 将来DB候補 | `asset_usages`、`character_usages` |
| 備考 | 「素材棚に追加済み」とは異なり、ゲーム内ノード/シーンで実際に参照されている状態を指す。まだ使っていない素材を「引用」と呼ぶと混同しやすいため命名に注意 |
...
| 素材棚に追加 | 制作に使う候補として保存 |
| この素材を使う | 制作用ライブラリに追加 |
| このゲームで引用している素材 | 実際にゲーム内で参照している素材 |
