...
未着手・一部実装・後続拡張のアイデア索引は `docs/IDEA_BACKLOG.md` を参照してください。
...
- 公開ゲーム
  ...
  - マイ一覧（管理）: `/my/games`
    ...
    - 公開前クレジット確認画面MVP（2026-05-06）
      ...
      - モーダル内容
        ...
        - 編集導線ボタン（警告ありの場合）
          ...
          - 直接ジャンプMVP（2026-05-12）: 削除済み/見つからない/非公開の素材・キャラクターカードで、該当する参照診断 issue を使って公開前チェックへ直接遷移可能
          ...
          - 複数使用箇所一覧MVP（2026-05-12）: 削除済み/見つからない/非公開の素材・キャラクターが複数ノードで使われている場合、カード内に「使用箇所 N件」と各行（Scene/Node/フィールド）を最大5件表示。各行に「ここへ移動」/「該当箇所へ移動」ボタン（1件のみの場合は「該当箇所へ移動」）。5件超は「ほかN件」表示。issue未特定時は「参照を編集」カテゴリ導線にフォールバック。
            - 入力フィールドそのものへの自動フォーカス、一括修正、自動差し替えは将来課題
        - キャンセルボタン（公開しない）
    ...
    - 公開中編集時の注意バナーMVP（2026-05-07）
      - 公開済みゲームを編集しても自動で非公開にはしない
      - 保存した変更は現行MVPでは公開版にも即反映される
      - 編集画面 (`/my/games/:id/edit`) で `isPublic === true` のとき、タイトル行直下に注意バナーを表示
      - バナー文言: 「このゲームは公開中です / 保存した変更は公開版にも反映されます。新しく追加された素材・キャラクターのクレジットは、保存時に公開時点の情報として固定されます。」
      - 公開済みゲームで新しく追加された素材/キャラクター参照の `GameCredit` は保存・参照同期時に即 `snapshotLockedAt` 付きでlock される
      - バナー折りたたみ機能MVP（2026-05-07）
        - 全文表示 / 省スペース表示（1行）の2状態を切り替えボタンで操作可能
        - 折りたたみ状態を `localStorage`（key: `talking.editor.publishedEditBannerCollapsed.v1`）に保存し、再読み込み後も維持
        - 全ゲーム共通の折りたたみ状態（ゲームIDによる分離は将来課題）
      - 公開中編集時の保存前再確認UX MVP（2026-05-08）
        - 編集画面のノード保存（「保存」「保存して次のノードへ」）で、`isPublic === true` のゲームのみ保存直前に `window.confirm` を表示
      ...
      - 公開中ゲームの構造変更confirm拡張 MVP（2026-05-09）
        ...
        - 公開中のノード削除・シーン削除では、公開中警告と削除参照summaryを1回のconfirm文言に統合して表示
      ...
      - ノード追加・シーン追加への公開中confirm拡張、差分検出、重要変更のみ確認、独自モーダル化、「今後表示しない」導線、公開版/下書き版分離は将来課題
...
- 公開前クレジット確認画面MVP（2026-05-06）
  ...
  - スタッフロール設定MVP（2026-05-10）を実装済み。`GameProject.staffRollEnabled`（default: `true`）でゲーム単位に導線の表示ON/OFFを切り替える。
    ...
    - OFFでも通常クレジット表示と `GET /games/:id/credits` は維持する既存仕様を変更しない。
    - 公開中ゲームで全体設定モーダル内の公開版に影響する変更を保存する場合は、共通confirmを表示する。
  ...
  - スタッフロール終了時挙動MVP（2026-05-12）を実装済み。ゲーム単位で自動スクロール末尾到達時の動作を選択できる。
    ...
    - `close`: 末尾に到達したらモーダルを自動的に閉じる。
    - `loop`: 末尾に到達したら先頭へ戻り、そのまま自動スクロールを継続する。軽い待機なしで自然に戻す。
    - `staffRollEnabled=false` でも終了時挙動の設定値は保存される（他の速度設定・表示順と同じ）。
    - 公開中ゲームで終了時挙動設定を変更して「全体設定を保存」する場合は既存の共通confirm対象。変更なし保存ではconfirmなし。非公開ゲームはconfirmなし。
    - `credits.counts.total === 0`（空表示）の場合は無理に終了時挙動を発火しないようにする。
    - ユーザーが明示的に一時停止している時は、末尾挙動を暴発させない。モーダル閉じた後に interval / timer が残らないようにクリーンアップする。
  - 以下は将来課題のまま。
    - スタッフロール拡張（より細かな速度カスタマイズ）
    - スタッフロール用BGM（なし / アップロード音源 / 既存音声素材から選択）
    - BGM音量
    - BGM/SE連動
    - より凝った演出（フェードイン/アウト、スクロール時の加速度など）
    - カテゴリ別アニメーション
    - BGM/SE連動
  - 設計メモ（将来課題）
    - スタッフロール3段階速度設定MVP（slow=28px/sec、normal=42px/sec、fast=64px/sec）は実装済み。上記は拡張項目として段階的に検討する。
    - スタッフロール用BGMは音声素材管理・権利表示・`GameCredit`・公開前確認と整合する必要があるため、単独追加ではなく関連仕様をまとめて確認してから実装する。
    - 現時点では以下が実装済みで、その他は未実装の将来課題として扱う：3段階速度設定UI（ゆっくり/標準/速い）、ゲーム全体設定での保存、設定値の保存状態。
...
- 公開ゲーム
  ...
  - 公開プレイの終端では、現在ノードに `nextNodeId` がなく選択肢もないとき、本文表示完了後の次操作（クリック/Enter/自動進行）で読了後カードを表示する
...
- `ownerId` UUID の丸出し表示をやめ、`by d7ef...f292` のように先頭4文字＋末尾4文字に短縮表示する。
- 素材名/キャラ名を1行目に出し、作者表示と用途バッジを2行目に小さめ表示する。
- 用途（`背景 4箇所` / `BGM 2箇所` など）は文字列連結から小さな badge/pill 表示に変更。
- `linkable === false`（削除済み/非公開/不明）の項目は名前を控えめ色にし、作者行を「詳細非公開」と表示する。
...
- ~~公開前確認画面からの編集導線強化 MVP~~ → **実装済み（2026-05-07）** — クレジット確認モーダルから編集画面の公開前チェックへ移動できる導線を追加。警告ありの場合に警告サマリーボタン「問題を確認する」、素材/キャラクター別カードのボタン「参照を編集」を実装。クリックで `/my/games/{id}/edit` へ遷移し、カテゴリ単位で公開前チェックへ誘導可能。
- ~~公開前チェックサマリーMVP~~ → **実装済み（2026-05-14）** — 公開前チェックパネル（編集画面右ペイン）の issue 一覧より上にサマリーカードを追加。全体状態として `公開準備OK` / `注意あり` / `要修正` を表示（判定順: `scenarioCheckCounts.error > 0` → 要修正、`referenceDiagnosticsError` あり → 注意あり（参照診断取得失敗を含む）、`scenarioCheckCounts.warning > 0` → 注意あり、`referenceDiagnosticsLoading` → チェック中…、それ以外 → 公開準備OK）。severity 別件数（要修正 N件 / 注意 N件 / 情報 N件）と既存カテゴリ別件数（構成 / 素材参照 / キャラクター参照）を表示。要修正がある場合は error 系 issue を最大 3 件表示し、3 件超は「ほか N件」で省略。issue 一覧への `#publish-check-issues` アンカー導線付き。既存フィルター・カテゴリフィルター・「対象へ移動」導線は維持。自動修正・一括修正・入力フィールドへのピンポイントフォーカスは未実装（将来課題）。DB/API変更なし。
- ~~公開前確認画面内の修正候補表示 MVP~~ → **実装済み（2026-05-07）** — クレジット確認モーダルの異常status項目に修正候補ボックスを表示。素材 `missing/deleted/private`、キャラクター `missing/deleted/private` に対応。直接ジャンプMVP（2026-05-12）・複数使用箇所一覧MVP（2026-05-12）は実装済み。入力フィールドへのピンポイントフォーカス・一括修正・自動差し替えは将来課題。
...
- クレジット確認からの直接ジャンプ拡張（最初の該当issueへの直接ジャンプMVP・複数使用箇所一覧MVPは実装済み。5件超の展開/折りたたみ、入力フィールドへのピンポイントフォーカス、一括修正、自動差し替えは将来課題）
- 作者向けテストプレイ支援（MVP実装済み、作者専用UI）
  ...
  - 実装済み（MVP）:
    - 編集画面でテストプレイ導線を2つ提供（`選択中からテスト` / `最初からテスト`）。どちらも `testPlay=1` を付与し、新規タブで開く。
    - `選択中からテスト` は現在選択中のシーン/ノードを優先して開始。ノード未選択時はシーン開始ノード（`scene.startNodeId`）→ シーン先頭ノードを解決し、シーンも未選択ならゲーム全体の開始位置へフォールバックする。
    - `最初からテスト` は選択中シーン/ノードを無視し、ゲーム全体の開始位置（`startSceneId` + その開始ノード）から開始する。
    - どちらも保存済み内容で再生するため、未保存の `nodeDraft` は反映しない。
    - テストプレイ中の簡易デバッグ表示（現在シーン/現在ノード/次ノード/選択肢数/選択肢遷移先概要/使用素材ID/キャラクター数）。
    - テストプレイ中の遷移ログ表示（メモリ内保持、最大30件、通常/フルスクリーン両方に表示）。
    - 遷移ログはメモリ保持のみで、DB / API / localStorage には保存しない（永続化ではなく、現在のメモリ上の最大30件を対象）。
    - 遷移ログはテストプレイパネルからテキストコピーできる。
    - 遷移ログはJSONファイルとしてエクスポートできる（ただし永続化ではなく、現在のメモリ上の最大30件を出力するだけ）。
    - コピー / エクスポート操作そのものは遷移ログに記録しない。
    - `start` / `next` / `choice` / `end` / `missing` を記録する。
    - 遷移ログの手動クリア（ゲーム進行には影響しない）。
    - テストプレイパネルの折りたたみ/展開。
    - テストプレイパネルの情報整理MVP（現在地 / 操作 / ノード情報 / 遷移ログの4セクション化、通常/フルスクリーンで同一構造）。
    - テストプレイから編集画面へ戻る導線（`/my/games/:id/edit?sceneId=...&nodeId=...`）。
    - テストプレイ中の全文表示ボタン（現在ノードの本文をタイプライター待ちなしで即座に全文表示）。
    - テストプレイ中の高速確認 ON/OFF トグル。
    - 高速確認 ON 中は現在ノードと以後のノード本文をタイプライター待ちなしで即表示。
    - 高速確認モード状態は localStorage に保存し、次回以降の作者向けテストプレイでも復元する。
    - 永続化対象は高速確認ON/OFFのみ。全文表示の単発状態や testPlayInstantRevealNodeId は保存しない。
    - 遷移ログは引き続きメモリ保持のみで、DB / API / localStorage には保存しない（永続化ではなく、現在のメモリ上の最大３０件を対象）。
    - DB / API には保存しない。
    - 高速確認ON/OFF操作は遷移ログに記録しない。
    - 高速確認 ON でも選択肢自動選択や自動進行はしない。
    - テストプレイ中の「選択肢までスキップ」ボタン（通常/フルスクリーン両方に表示）。
    - 「選択肢までスキップ」は現在ノードから `nextNodeId` を連続でたどり、最初の選択肢ノードで停止する。停止時は到達先ノード本文をタイプライター待ちなしで即全文表示してから選択肢を表示し、選択肢の自動選択はしない。
    - スキップ中の通過遷移は既存の `next` / `end` / `missing` ログで記録し、ボタン押下操作そのものは遷移ログに記録しない。
    - 「選択肢までスキップ」はループ対策で最大ステップ数ガードを持ち、到達時は警告して停止する。
    - 「選択肢までスキップ」操作状態はメモリ保持のみで、DB / API / localStorage には保存しない。
  - 将来課題:
    - 分岐までスキップ。
    - 自動巡回/自動到達確認。
    - フラグ/変数インスペクタ（`flags` / `variables` / `visitedNodes` / `affinity` など）。
    - 遷移ログの永続化。
    - テストプレイ専用警告（開始地点未設定・参照切れ等）の拡張。
- ライブ編集 / プレイしながら作るモード（中長期構想、編集体験v2）
  - 位置づけ:
    - 通常の3ペイン編集画面が「データを組み立てる」画面であるのに対し、本構想は「作品を再生しながら、その場で続きを作る」ための将来モード。
    - 全画面に近いプレイ画面を中心に、左右いずれかのスライドパネルで作者向け編集操作を重ねる体験を想定する。
    - 今すぐ実装する確定仕様ではなく、作者向けテストプレイ支援MVPのさらに先で検討する将来課題として扱う。
  - 目的:
    - 編集画面とテストプレイ画面の往復を減らす。
    - 作者がプレイヤー視点のままシナリオを伸ばせるようにする。
    - 「この場面の次を作る」感覚でノード追加・選択肢追加・遷移先設定を行えるようにする。
    - ノベルゲーム制作をフォーム入力中心から、プレイ体験に近い創作体験へ寄せる。
    - テストプレイ支援機能と連携し、確認しながら作れる導線を作る。
  - 想定機能（候補）:
    - 全画面または大きめのプレイプレビュー。
    - 左右から開く作者用スライドパネル。
    - 現在シーン / 現在ノードの表示。
    - 現在ノードの簡易編集（本文、話者、背景、キャラクター配置、BGM/SEなど将来の演出項目）。
    - 現在位置から次ノードを作成。
    - 選択肢追加。
    - 選択肢ごとの遷移先ノード作成。
    - その場で再生確認。
  ...
  - テストプレイ支援との関係整理:
    - テストプレイ支援MVP（先行段階）:
      ...
      - 遷移ログ。
      - デバッグ表示。
    - ライブ編集 / プレイしながら作るモード（後続段階）:
      - テストプレイ支援の上に編集操作を重ねる中長期構想。
      - プレイ確認中にノード作成・編集・遷移設定まで行う。
      - 実装難易度が高いため、テストプレイ支援MVPより後の段階で検討する。
  - 設計課題・注意点:
    - 実装難易度は高め。
    - 既存 `edit.vue` の3ペイン編集画面との責務分離が必要。
    - 通常編集画面と機能が二重化しすぎないようにする。
    - 未保存変更の扱いが難しい。
    - プレイ中状態と編集中状態の同期が必要。
    - 現在ノード編集後にプレイプレビューへ即反映する設計が必要。
    - 次ノード作成・選択肢作成・遷移先設定の一体化が必要。
    - 公開中ゲームでは変更が公開版へ即反映されるかどうかの確認UXが必要。
    - undo/redo やキャンセル導線が必要になる可能性が高い。
    - 公開版/下書き版分離や差分検出とも関係する。
    - まずはテストプレイ支援MVPを先に進め、その後の中長期構想として扱う。
- より精密なノード/フィールド単位ジャンプの拡張（最初の該当issueへの直接ジャンプMVP・複数使用箇所一覧MVPは実装済み。5件超の展開/折りたたみ、入力フィールドへのピンポイントフォーカスは将来課題）
- 公開中編集時の再確認UX拡張（ノード追加・シーン追加への公開中confirm拡張、差分検出、重要変更のみ確認、独自モーダル化、「今後表示しない」導線、公開版/下書き版分離）
...
- **表示箇所:**
  - 公開アセット詳細 `/assets/:id`: 「利用条件」セクション（クレジット必須/任意バッジ + 自由テキスト）— 2026-05-21 実装済み（`/characters/:id` と同じ見た目に統一）
...
- 現状:
  - 2026-05-05 UI polish で `ownerId` の丸出し表示をやめ、`by d7ef...f292` 形式に短縮済み。
  - プロフィール/クリエイター名MVPで、公開ゲーム一覧・詳細・クレジット表示は `ownerDisplayName` 優先表示に対応済み。
...
- 将来課題:
  ...
  - **クレジット運用系**
    - より精密なノード/フィールド単位ジャンプの拡張（最初の該当issueへの直接ジャンプMVP・複数使用箇所一覧MVPは実装済み。5件超の展開/折りたたみ、入力フィールドへのピンポイントフォーカスは将来課題）
    - クレジット確認画面内の修正候補表示の拡張（ノード/フィールド単位の具体ガイド、一括修正候補、自動差し替え提案。最初の該当issueへの直接ジャンプMVPは実装済み）
    - 公開中編集時の再確認UX拡張（ノード追加・シーン追加への公開中confirm拡張、差分検出、重要変更のみ確認、独自モーダル化、「今後表示しない」導線、公開版/下書き版分離）
...
| `ASSET_PRIVATE` | 背景/BGM/SE | 非公開 | アセットが非公開状態（`isPublic=false`） |
...
- 3ペイン幅（scenes/nodes/props）は LocalStorage に永続化し、復元時に現在の表示幅に合わせてクランプする
  - キー: `gameEditorPaneWidths`
  - 壊れた値・過大値・過小値は安全値へ補正する
  - window resize 時にも再クランプして、表示幅を超える横スクロール暴れを防ぐ
  - 既存の3ペイン可変幅、`fullscreenProps`、リサイズ保存挙動は維持する
- 右ペイン上部に「表示設定をリセット」導線を配置する（MVP）
  - リセット対象（LocalStorage）
    - `gameEditorPaneWidths`（3ペイン幅）
    - `talking.editor.rightPaneSections.v1`（右ペインセクション開閉状態）
    - `talking.editor.lastSelection.v1:${gameId}`（現在ゲームの最後の選択位置）
  - 実行前に `window.confirm` で確認し、キャンセル時は何もしない
  - 実行後はページ再読み込みなしで現在画面へ反映する
    - 3ペイン幅は既定値へ戻して再保存（クランプ適用）
    - 右ペインセクション開閉は既定値へ戻して再保存
    - 制作ガイドの非表示状態も解除する
    - 最後の選択位置は削除（現在選択中のシーン/ノードはその場では変更しない）
  - この操作でゲーム内容（シーン/ノード/素材参照/キャラクター参照）は変更しない
  - 対象外（今回リセットしない）
    - `talking.editor.publishedEditBannerCollapsed.v1`
    - `talking_copy_opts_v1`
    - `talking.testPlay.fastConfirm.v1`
    - 音声同意など、表示崩れ復旧に直接関係しないユーザー設定
...
- `制作ガイド` は右ペイン上部アクション群の下、公開前チェックパネルの前に配置する軽量ナビ。強制チュートリアルではなく、作品状態に応じた「次にやると良さそう」な案内として表示する
  - `sectionOpen.guide` で折りたたみ可能。開閉状態は `talking.editor.rightPaneSections.v1` に保存され、通常表示/全画面表示で共有する
  - ゲーム単位の非表示状態は `talking.editor.creationGuideHidden.v1:${gameId}` に保存し、右ペインの設定グループから再表示できる
  - 表示設定リセットで制作ガイドの非表示状態も解除する
...
- 2026-05-18 に Phase 2-f を実装し、`EditorPublishCheckSummaryCard.vue` で公開前チェックサマリーカードを共通コンポーネント化した。表示用 computed（`publishCheckStatus` 系・`publishCheckTopIssues` 系）をコンポーネント側へ移譲。issue一覧・フィルタ・参照診断 API 処理・`sectionOpen.scenarioCheck` は `edit.vue` 側に維持。公開前チェックパネル全体のコンポーネント化は未着手
...
- シナリオ Import/Export
- 公開前チェック専用画面
- 公開前チェックUIの完全ミニマル化（将来課題・今回のMVPでは未実装）
  - 現状: ヘッダーに重大度件数バッジ（エラー/警告/情報）、展開部に重大度フィルタ行＋カテゴリフィルタ行
  - 将来案: ヘッダーの重大度件数バッジをクリック可能フィルタにし、展開部の重大度フィルタ行を削除する
    - 展開部はカテゴリフィルタ＋issue一覧を中心にする
    - スペースを節約しつつ、エラー/警告/情報への絞り込みは維持する
  - 現在のUIは許容とし、将来のUI整理タイミングで検討する
...
- ノード参照切れ警告・到達不能ノード検出は公開前チェックで実装済み。フローチャート可視化、意図的未使用ノードの扱い、より高度な到達性ルールは将来課題
- シーン/ノードコピー・移動は将来課題
