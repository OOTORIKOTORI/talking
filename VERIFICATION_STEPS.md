...
## お気に入り(アセット/キャラクター)
1. 公開ギャラリー `/assets` で任意カードの ♡ をON/OFF → トースト表示
2. `/my/favorites` の「アセット/キャラクター」タブを切り替え、登録内容が反映されること
3. リロードしても状態が保持されること

## タブ統合
1. 公開ギャラリー・アセット管理・アップロード・お気に入りで**アセット/キャラクター**のタブが出ていること
2. タブ切替時、検索UI/並び替え/URLクエリが破綻しないこと

## キャラクター編集
1. 立ち絵カードの**ドラッグ並べ替え**でsortOrderが更新されること
2. 画像クリックで拡大プレビュー、入力欄クリックでは発火しないこと
